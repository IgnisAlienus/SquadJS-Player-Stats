...
export default class DiscordPlayerStats extends DiscordBasePlugin {
  ...
  async onMessage(message) {
    try {
      ...
      this.verbose(1, 'Error in onMessage:', error);
      return;
    }
  }
  ...
  async postUserStats(steamID) {
    this.verbose(2, 'Posting User Stats for SteamID:', steamID);
    try {
      const daysAgo = moment()
        .subtract(this.options.daysBackToQuery, 'days')
        .toDate();
      ...
      // Get Player
      this.verbose(2, 'Getting Player Info for SteamID:', steamID);
      const playerResult = await this.models.Player.findOne({
        where: {
          steamID: steamID,
        },
      ...
      // Top Victim
      this.verbose(2, 'Calculating Top Victim for SteamID:', steamID);
