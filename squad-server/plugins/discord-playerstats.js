...
export default class DiscordPlayerStats extends DiscordBasePlugin {
  ...
  static get optionsSpecification() {
    return {
      ...
      inDiscordAllStatsCommand: {
        required: false,
        description: 'Command Players use in Discord to see all their stats.',
        default: 'allmystats',
      },
      ...
      serverTitle: {
        required: false,
        description: 'Title of the Server in the Embed.',
        default: 'Squad Server Stats',
      },
      statCooldown: {
        required: false,
        description:
          'Command Cooldown time in minutes. Prevents Users from Spamming the command.',
        default: 15,
      },
      ...
      dailyAllStatsManualPostCmd: {
        required: false,
        description: 'Command to manually send the Daily All Stats in Discord.',
        default: 'allstats',
      },
  ...
  async mount() {
    ...
    if (this.options.enableInGameStatsCommand === true) {
      this.server.on(
        `CHAT_COMMAND:${this.options.inGameStatsCommand}`,
        this.onStatCommand
      );
    }
    if (this.options.enableInDiscordStatsCommand === true) {
      this.server.on(
        `CHAT_COMMAND:${this.options.linkInGameAccountCommand}`,
        this.onLinkCommand
      );
    }
  ...
  async unmount() {
    ...
    this.server.removeEventListener(
      `CHAT_COMMAND:${this.options.statsCommand}`,
      this.onStatCommand
    );
    this.server.removeEventListener(
      `CHAT_COMMAND:${this.options.linkInGameAccountCommand}`,
  ...
  async onStatCommand(info) {
    ...
    const Wounds = await this.models.Wound.findAndCountAll({
      where: {
        attacker: steamID,
        time: {
          [Op.and]: [{ [Op.gte]: daysAgo }, { [Op.ne]: null }],
        },
        teamkill: false,
      },
    });
    ...
    // Calculate K/D
    const kdRatio =
      Deaths.count !== 0 ? (Kills.count / Deaths.count).toFixed(2) : 0;
  ...
  async onMessage(message) {
    ...
    const allmanualCmdRegex = new RegExp(
      '^!' + this.options.dailyAllStatsManualPostCmd + '$'
    );
    ...
    const allmystatsCmdRegex = new RegExp(
      '^!' + this.options.inDiscordAllStatsCommand + '(?:\\s+(\\d{17}))?$'
    );
    ...
    if (
      ...
      await this.postDailyStats('limited');
      return;
    } else if (
      message.content.match(manualCmdRegex) &&
    ...
    if (
      message.content.match(allmanualCmdRegex) &&
      this.options.enableDailyStats === true
    ) {
      if (message.member._roles.includes(this.options.dailymanualCmdRole)) {
        return message.reply('You do not have permission to use this command.');
      }
      await this.postDailyStats('all');
      return;
    } else if (
      message.content.match(allmanualCmdRegex) &&
      this.options.enableDailyStats === false
    ) {
      return message.reply('All Daily Stats are not enabled.');
    }
    if (
      message.content.startsWith(
        prefix + this.options.inDiscordAllStatsCommand
      ) &&
      this.options.enableInDiscordStatsCommand === true
    ) {
      const steamID = message.content
        .slice(prefix.length + this.options.inDiscordAllStatsCommand.length)
        .trim(' ');

      if (steamID) {
        await this.postUserStats(steamID, 'all');
        return;
      }
      ...
      await this.postUserStats(playerSteamID, 'all');
      return;
    } else if (
      message.content.match(allmystatsCmdRegex) &&
    ...
    if (
      message.content.startsWith(prefix + this.options.inDiscordStatsCommand) &&
      this.options.enableInDiscordStatsCommand === true
    ) {
      const steamID = message.content
        .slice(prefix.length + this.options.inDiscordStatsCommand.length)
        .trim(' ');

      if (steamID) {
        await this.postUserStats(steamID, 'limited');
        return;
      }
      const playerResult = await this.models.Player.findOne({
        where: {
      ...
      await this.postUserStats(playerSteamID, 'limited');
      return;
    } else if (
  ...
  async scheduleDailyStats() {
    ...
    const nextScheduledTime = moment
      .utc()
      .add(delay, 'milliseconds')
      .format('YYYY-MM-DD HH:mm:ss');
    this.verbose(
      1,
      `Next scheduled time for daily stats: ${nextScheduledTime}`
    );
  ...
  async postDailyStats(type) {
    const isAllStats = type === 'all';
    const daysAgo = moment()
      .subtract(this.options.daysBackToQuery, 'days')
      .toDate();
    ...
    const result = await this.models.Death.findOne({
      where: {
        ...(!isAllStats && { time: { [Op.gte]: daysAgo } }),
    ...
    const title = isAllStats
      ? `All ${this.options.serverTitle} Stats`
      : `${
          this.options.serverTitle
        } Stats for the Last ${this.options.daysBackToQuery.toString()} Days`;
  ...
  async postUserStats(steamID, type) {
    console.debug(steamID);
    const isAllStats = type === 'all';
    const daysAgo = moment()
      .subtract(this.options.daysBackToQuery, 'days')
      .toDate();
    ...
    const topVictimResult = await this.models.Death.findOne({
      ...
      attributes: [
        'victimName',
        [Sequelize.fn('COUNT', Sequelize.literal('victim')), 'Count'],
      ],
      group: ['victimName'],
    ...
    const topNemesisResult = await this.models.Death.findOne({
      ...
      attributes: [
        'attackerName',
        [Sequelize.fn('COUNT', Sequelize.literal('attacker')), 'Count'],
      ],
      group: ['attackerName'],
      order: [[Sequelize.literal('Count'), 'DESC']],
      limit: 1,
